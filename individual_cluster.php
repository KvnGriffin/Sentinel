...
<?php 
     ...
     echo "<a href='#zeros'>".$highest_term[0]."</a>";
     echo "<br>";
     echo "<a href='#ones'>".$highest_term[1]."</a>";
     echo "<br>";
     echo "<a href='#twos'>".$highest_term[2]."</a>";
     echo "<br>";
     echo "<a href='#threes'>".$highest_term[3]."</a>";
     echo "<br>";
     echo "<a href='#fours'>".$highest_term[4]."</a>";
     echo "<br>";
	 echo "<a href='#fives'>".$highest_term[5]."</a>";
     echo "<br>";
     echo "<a href='#sixs'>".$highest_term[6]."</a>";
     echo "<br>";
?>
...
<div id="zeros">	   
  <?php 
    echo "<br>";
    echo "<big><b>".$highest_term[0]."</b></big>  <a href='#Top'> Back to Top</a>";
    for($z=0; $z<50;$z++)
    {
	foreach($clusters[0]['members'][$z] as $url => $tf)
    {
        $test = "$url";
		$clus_bing="SELECT * FROM Bing WHERE Url='$test'";
...
<?php
   ...
   /***** Pull data into array $bing as key => value pairs($url2 => $description) */
       while($row2 = mysql_fetch_assoc( $result_blekko))
        {
	       $url2 =$row2['Url'];
	       $description2 = $row2['Description'];
	       $blekko[$url2] = $description2;
        }
    ...
    * hoped it might help
    * with speed*/   
    foreach($blekko as $url => $string)
    {
	  $punctuation2[$url] = ereg_replace("[^A-Za-z _]", "", $blekko[$url]);
    }
    
    foreach($punctuation2 as $url => $string)
    {
		$collection2[$url] = stopwords($punctuation2[$url]);
	}
...
/*** Array $words contains all words(lowercase and trimmed of any white space in $collection   ***/
  foreach($collection2 as $url => $strin)
	{
	  $each_url2 = explode(" ", strtolower($strin));
		  foreach($each_url2 as $ul => $wrd)
		   {
		     $words2[] = strtolower(trim($each_url2[$ul]));
		   }
	}

/**** array_unique removes all duplicates from $word ****/
    $ind_words2 = array_unique($words2);
	$num_ind2 = count($ind_words2);
  
    foreach($collection2 as $dat)
	{
		$trimmed2 = strtolower(trim($dat));
		$words_in_doc2 = explode(" ",$trimmed2);
		$ind_words_doc2 = array_unique($words_in_doc2);
		     
		  foreach($ind_words_doc2 as $indwrds)
		  {
            foreach($words_in_doc2 as $wrds) 		 
		    {
			 if (strcasecmp(trim($indwrds), trim($wrds)) == 0)
			 $count_ind_words2[$indwrds]++;
			}
	      }
	}
	
	$count_ind_words_highest2 = $count_ind_words2;
		arsort($count_ind_words_highest2);
      //  echo "Highest count: ";
        
    $max_count2 = reset($count_ind_words_highest2); 
       // echo $max_count;
	 
	 /**************************** Term frequencies ***********************
	  * counts number of occurencies of each individual word associated with each url  */
	
	foreach($collection2 as $col => $st)
	{
		$string2 = strtolower($st);
	    $url12 = explode(" ",$string2);
	    foreach($url12 as $u)
	    {
	    foreach($ind_words2 as $indwrds)
	    {
		     if (strcasecmp(trim($indwrds), trim($u)) == 0)
			 $count_ind_words_url2[$col][$indwrds]++;
	 ...
	 * counts the number of documents in which a word appears ********/
		foreach($collection2 as $dat)
		{
		        ...
		        $words_in_doc2 = explode(" ",$trimmed2);
		    $ind_words_doc2 = array_unique($words_in_doc2);
		    $ind_wor_line2[] = implode(" ", $ind_words_doc2);
		 
		}
		
		 foreach($ind_wor_line2 as $iwl)
		 {
			$trimmed2 = strtolower(trim($iwl));
		    $words_in_doc2 = explode(" ",$trimmed2);
		    $ind_words_doc2 = array_unique($words_in_doc2);
		  ...
		  foreach($ind_words_doc2 as $indwrds)
		  {
		   ...
		   {
			 if (strcasecmp(trim($indwrds), trim($wrds)) == 0)
			 $doc_freq_per_term2[$indwrds]++;
			 
		   }
	      }
		 }
		 
		 $num_of_urls2 = count($collection2);   // number of url's to be used in idf calculation.
	  
	  /***************** tf-idf ********************************/
	 //  echo "<br>";
	foreach($count_ind_words_url2 as $url => $wrd)
	{
	
	   foreach ( $wrd as $key => $val)
	   {  
			$count_ind_words_url3[$url][$key] = $count_ind_words_url2[$url][$key] * log($num_of_urls2/$doc_freq_per_term2[$key],2);
	   }
	}
	 ...
	 * so the new array will have the structure [url][url] => tf_idf(score) */
	$coll_tf_idf2 = array();
	  foreach ($collection2 as $url => $string)
	  {
		  $coll_tf_idf2[$url][$url] = $count_ind_words_url3[$url];
	  }
	// echo "This is: coll_tf_idf ";
	// echo "<br>";
	// var_dump($coll_tf_idf);
     ...
     * which is then placed into 'centroid' when array $clusters is created */
	 ...
	 $kmeans = 7;
	 $coll_tf_idf_rand2 = array();
	 $coll_tf_idf_rand2 = $coll_tf_idf2;
	 for($i=0; $i<$kmeans; $i++)
	 {
		 $rand2[$i] = array_rand($coll_tf_idf_rand2);
		 $centroids2[$i] = $coll_tf_idf_rand2[$rand2[$i]][$rand2[$i]]; 
	 }

      
     $clusters= array();
    // $clusters is each cluster
	 $k=0;
	 
	for($k=0;$k<$kmeans;$k++)
	{
	  $clusters[$k] = array( 'centroid' => $centroids2[$k],
	                    'members' => array()
	                    );
    }
	...
	{ 
	    $index = 0;
		$cluster_index= 0;
		foreach($coll_tf_idf_rand2 as $url)
		{
			foreach($url as $url2)
			{
			 $max_sim=0;
  ...
  /* Cluster names use word with highest tf-idf score for each cluster member array 
   * $highest-term holds the name with each ith element corresponding to it's cluster number  */
...
<div id="zero">	   
  <?php 
    echo "<br>";
    echo "<big>".$highest_term[0]."</big>  <a href='#Menu2'> Back to Top</a>";
   $count = count($clusters[0]['members']);
    ...
    {
        $test1 = "$url";
		$clus_blekko="SELECT * FROM Blekko WHERE Url='$test1'";
        $clusblekko=mysql_query($clus_blekko);
        $row1 = mysql_fetch_assoc($clusblekko);
        
	     echo "<p><a href='".$row1['Url']."'>".$row1['Title']."</a><br /><i>".$row1['Description']."</i><br /><span style='font-size: 10pt;'>".$row1['DisplayUrl']."</span></p>"; // display everything
    }
    }
    ?>
</div> 
...
<div id="one">	   
  <?php 
  echo "<br>";
  echo "<big><b>".$highest_term[1]."</b></big><a href='#Menu2'> Back to Top</a>";
  for($z=0; $z<50;$z++)
    {
	foreach($clusters[1]['members'][$z] as $url => $tf)
    {
        $test2 = "$url";
		$clus_blekko="SELECT * FROM Blekko WHERE Url='$test2'";
        $clusblekko=mysql_query($clus_blekko);
        $row = mysql_fetch_assoc($clusblekko);
...
<div id="two">	   
  <?php 
  echo "<br>";
  echo "<big><b>".$highest_term[2]."</b></big><a href='#Menu2'> Back to Top</a>";
  for($z=0; $z<50;$z++)
    {
	foreach($clusters[2]['members'][$z] as $url => $tf)
    {
        $test3 = "$url";
		$clus_blekko="SELECT * FROM Blekko WHERE Url='$test3'";
        $clusblekko=mysql_query($clus_blekko);
        $row = mysql_fetch_assoc($clusblekko);
...
<div id="three">	   
  <?php 
  echo "<br>";
  echo "<big><b>".$highest_term[3]."</b></big><a href='#Menu2'> Back to Top</a>";
  for($z=0; $z<50;$z++)
    {
	foreach($clusters[3]['members'][$z] as $url => $tf)
    {
        $test4 = "$url";
		$clus_blekko="SELECT * FROM Blekko WHERE Url='$test4'";
        $clusblekko=mysql_query($clus_blekko);
        $row = mysql_fetch_assoc($clusblekko);
...
<div id="four">	   
  <?php 
  echo "<br>";
  echo "<big><b>".$highest_term[4]."</b></big><a href='#Menu2'> Back to Top</a>";
  for($z=0; $z<50;$z++)
    {
	foreach($clusters[4]['members'][$z] as $url => $tf)
    {
        $test5 = "$url";
		$clus_blekko="SELECT * FROM Blekko WHERE Url='$test5'";
        $clusblekko=mysql_query($clus_blekko);
        $row = mysql_fetch_assoc($clusblekko);
...
<div id="five">	   
  ...
  for($z=0; $z<50;$z++)
    ...
    {
        $test = "$url";
		$clus_bing="SELECT * FROM Blekko WHERE Url='$test'";
        $clusbing=mysql_query($clus_bing);
        $row = mysql_fetch_assoc($clusbing);
...
<?php 
     ...
     echo "<a href='#zeross'>".$highest_term[0]."</a>";
     echo "<br>";
     echo "<a href='#oness'>".$highest_term[1]."</a>";
     echo "<br>";
     echo "<a href='#twoss'>".$highest_term[2]."</a>";
     echo "<br>";
     echo "<a href='#threess'>".$highest_term[3]."</a>";
     echo "<br>";
     echo "<a href='#fourss'>".$highest_term[4]."</a>";
     echo "<br>";
	 echo "<a href='#fivess'>".$highest_term[5]."</a>";
     echo "<br>";
     echo "<a href='#sixss'>".$highest_term[6]."</a>";
     echo "<br>";
?>
