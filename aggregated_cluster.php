...
<?php
    ...
    /************  Select data from individual tables  ***************/
    ...
    /***** Pull data into three arrays as key => value pairs($url => $score)  for use in Borda function ******/ 
  ...
  /******************** Aggregate all three tables into one using borda *******/
    $temp = Borda($bing, $yahoo);
    $temp2 = Borda($blekko, $temp);
    /*** Sort by value and then "pop" last element from array that is " " => NULL ****/
    arsort($temp2);
    $end_element = array_pop($temp2);
   
    
    /**** $temp2 no has aggregated data from all three tables. Next foreach loop SELECT's Url's and 
     * Descriptions from database that relate to each url in $temp2. Array $aggregated contains url's 
     * to descriptions as key => value pairs. ****/ 
    
    foreach($temp2 as $url4 => $score3)
        {
	     $test = "$url4";
		 $agg_yahood="SELECT * FROM Yahoo WHERE Url ='$test' UNION SELECT * FROM Bing WHERE Url='$test' UNION SELECT * FROM Blekko WHERE Url ='$test'";
         $agg_yahoo=mysql_query($agg_yahood);
         $row2 = mysql_fetch_assoc($agg_yahoo);
        
	       $url2 =$row2['Url'];
	       $description = $row2['Description'];
	       $aggregated[$url2] = $description;
        }
    ...
    * hoped it might help
    * with speed*/
    foreach($aggregated as $url => $string)
    {
	  $punctuation[$url] = ereg_replace("[^A-Za-z _]", "", $aggregated[$url]);
    }
  ...
  foreach($collection as $url => $strin)
   ...
   /**** array_unique removes all duplicates from $word ****/
     ...
     * which is then placed into 'centroid' when array $clusters is created*/
     ...
     $clusters= array();  // $clusters is each cluster plus members
	 $k=0;
...
<!-- Could not figure out a way to have each cluster name as a clickable link on the left and display
 ...
 rewritten, which is not desirable, but I think that's more to do with my lack of HTML skills and 
...
<div id="zero">	   
  <?php 
    ...
    {
        $test = "$url";
		$clus_bing="SELECT * FROM Yahoo WHERE Url ='$test' UNION SELECT * FROM Bing WHERE Url='$test' UNION SELECT * FROM Blekko WHERE Url ='$test'";
